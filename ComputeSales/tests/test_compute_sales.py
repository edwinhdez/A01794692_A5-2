...
class TestMain(unittest.TestCase):
    def setUp(self):
        ...
        if not os.path.exists(self.sales_file):
            with open(self.sales_file, 'w') as f:
                f.write('[{"product": "product1", "quantity": 2}, {"product": "product2", "quantity": 1}, {"product": "product3", "quantity": 3}]')
    ...
    def test_main(self):
        ...
        result = subprocess.run(['python', 'C:/git/TecMonterrrey/QualitySoftware/A01794692_A5-2/ComputeSales/src/computeSales.py', self.catalogue_file, self.sales_file], capture_output=True, text=True)
    ...
    def test_file_not_found(self):
        ...
        result = subprocess.run(['python', 'C:/git/TecMonterrrey/QualitySoftware/A01794692_A5-2/ComputeSales/src/computeSales.py', 'non_existent_catalogue.json', 'non_existent_sales.json'], capture_output=True, text=True)
        output = result.stdout.strip()
